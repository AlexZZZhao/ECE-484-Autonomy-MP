...
def line_fit(binary_warped):
	...
	for window in range(nwindows):
		...
		####
		# Identify the nonzgood_right_indsero pixels in x and y within the window
		## TODO
		good_left_inds = []
		good_right_inds = []
	
		for i in range(nonzerox.shape[0]):
			if(nonzerox[i] >=  win_xleft_low and nonzerox[i] < win_xleft_high and nonzeroy[i] >= win_y_low and nonzeroy[i] < win_y_high):
				good_left_inds.append(i)
			elif(nonzerox[i] >=  win_xright_low and nonzerox[i] < win_xright_high and nonzeroy[i] >= win_y_low and nonzeroy[i] < win_y_high):
		...
		if len(good_left_inds) > minpix and np.int_(np.mean(nonzerox[good_left_inds])) < midpoint:
			leftx_current = np.int_(np.mean(nonzerox[good_left_inds]))

		if len(good_right_inds) > minpix and np.int_(np.mean(nonzerox[good_right_inds])) > midpoint:
			rightx_current = np.int_(np.mean(nonzerox[good_right_inds]))
		####

	# Concatenate the arrays of indices
	#left_lane_inds = left_lane_inds.astype('uint8')
	#right_lane_inds = right_lane_inds.astype('uint8')
	...
	#print("rightx:", rightx, np.mean(rightx), np.mean(leftx))
